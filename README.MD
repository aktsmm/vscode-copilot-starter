...
- [補足情報](#補足情報)
  ...
  - [GitHub Copilot プラン比較ガイド](docs/copilot-plans.md)
  - [組織ポリシーによる機能制限](docs/organization-policies.md)
  - [Azure CLI の権限とロール](docs/azure-permissions.md)
  - [プロキシ環境での設定（プレビュー）](docs/proxy-settings.md)
  - [ファイアウォール・通信要件](docs/network-requirements.md)
...
| [GitHub Copilot ライセンス](https://github.com/features/copilot) | AI コード補完・Chat・Agent         | Free（無料）/ Pro / Pro+ / Business / Enterprise     |
| [Azure アカウント](https://azure.microsoft.com/ja-jp/free/)      | Bicep デプロイ、Azure リソース操作 | ⭐ 推奨（なくても Bicep の編集・構文チェックは可能） |

> 💡 **GitHub Copilot Free**（$0）で始められます。主に使う **Agent モード**（IDE 内）は Free でも利用可能（50 回/月）。詳細は [Copilot 料金ページ](https://github.com/features/copilot) を参照。
>
> 📖 **プラン比較・エージェント機能の違い**: [GitHub Copilot プラン比較ガイド](docs/copilot-plans.md) を参照。
...
> 💡 ターミナルの開き方・実行ポリシーなどは [コマンドの実行場所](#-コマンドの実行場所) を参照。
...
> 💡 **Copilot 関連の追加拡張機能について**: GitHub Copilot for Azure など追加の拡張機能もありますが、インストール時に設定ファイル（`.vscode/mcp.json` 等）が自動生成されることがあります。上記の拡張機能だけで十分に Copilot を活用できます。
...
>
> 📖 **権限エラーが出る場合**: [Azure CLI の権限とロールについて](docs/azure-permissions.md) を参照してください。
...
<details>
<summary>方法 2: 手動設定（クリックで展開）</summary>
...
> ⚠️ **Organization ポリシーで MCP が制限されている場合**
>
> 企業や組織で管理されている環境では、MCP サーバーへのアクセスがポリシーで制限されていることがあります。  
> 設定 → `Chat > Mcp: Access` が「**organization によって管理されています**」と表示され、`none` に固定されている場合は MCP サーバーを利用できません。この場合は組織の IT 管理者に相談してください。
...
1. `Ctrl + Shift + I` で Copilot Chat を開く（または右側の Copilot アイコンをクリック）
...
🔗 [プロキシ環境での設定](docs/proxy-settings.md)
