...
- [事前準備（必須）](#-事前準備必須)
  - [VS Code のインストール](#1️⃣-vs-code-のインストール)
  - [VS Code 拡張機能の導入](#2️⃣-vs-code-拡張機能の導入)
  - [Azure CLI のインストール](#3️⃣-azure-cli-のインストール)
  - [Git CLI のインストール](#4️⃣-git-cli-のインストール)
  - [GitHub CLI のインストール](#5️⃣-github-cli-のインストール)
  - [MCP サーバー (MS Learn) の設定](#6️⃣-mcp-サーバー-ms-learn-の設定)
  - [準備完了の確認](#️-準備完了の確認)
...
> 💡 インストール時に「**Add to PATH**」にチェックを入れると、ターミナルから `code` コマンドが使えるようになります。
...
| [Azure Account](https://marketplace.visualstudio.com/items?itemName=ms-vscode.azure-account)                    | Azure アカウント管理                                  |
| [Azure Resources](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azureresourcegroups) | Azure リソースのブラウズ                              |
...
VS Code で任意のファイルを開き、コメントを入力すると補完候補が表示されます。
...
1. `Ctrl + Shift + I` で Copilot Chat を開く
